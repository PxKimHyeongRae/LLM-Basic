"""
QLoRA 파인튜닝 스크립트 v2 - 구조화된 프롬프트 사용
새로운 프롬프트 형식으로 모델을 파인튜닝합니다.
"""
...
def main():
    ...
    OUTPUT_DIR = "./finetuned_model_v2"

    # 대규모 데이터 사용 (300개)
    # 소규모 데이터 사용 시: train_structured.jsonl, validation_structured.jsonl
    TRAIN_FILE = "data/train_large.jsonl"
    VAL_FILE = "data/validation_large.jsonl"

    print("=" * 70)
    print(f"🚀 파인튜닝 v2 시작: {MODEL_NAME}")
    print("   새로운 구조화된 프롬프트 형식 사용")
    ...
    # LoRA 설정 - rank를 높여서 더 많은 파라미터 학습
    lora_config = LoraConfig(
        r=32,  # rank 증가 (16 -> 32)
        lora_alpha=64,  # alpha도 비례해서 증가
        target_modules=[
            "q_proj",
            "k_proj",
            "v_proj",
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=5,  # epoch 증가 (3 -> 5)
        per_device_train_batch_size=2,  # 배치 크기 증가 가능하면
        per_device_eval_batch_size=2,
        gradient_accumulation_steps=8,
        learning_rate=3e-4,  # learning rate 약간 증가
        fp16=True,
        logging_steps=5,
        eval_strategy="steps",
        eval_steps=20,
        save_strategy="steps",
        save_steps=20,  # eval_steps와 동일하게 변경 (50 -> 20)
        save_total_limit=3,
        warmup_steps=30,
